...
// A.Util.inherits(A.Explosion, A.MovingObject);
// Not sure why this was overwriting the methods directly on this object,
// but we've overwritten the only methods we care about anyways.
...
A.Explosion.prototype.draw = function (ctx) {
  ...
  for (var i = 0; i < this.NUM_SPARKS; i++) {
    ...
    ctx.fillRect(this.pos[0] + this.radius * Math.cos(angle),
      this.pos[1] + this.radius * Math.sin(angle), 4, 4);
  }
}
